...
export default function ListDistrictComponent(props: AboutProps) {
  const { districts } = props;
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const rows = districts.sort((a, b) => (a.name < b.name ? -1 : 1));
  var number = 0;
  ...
  const handleViewById = (id: any) => {
    dispatch(getDistrict(id,history));
  };
    ...
    <Grid item xs={12}>
      <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
        ...
        <Root sx={{ width: 1200, maxWidth: "100%" }}>
          <Button variant="outlined" size="small" href="/admin/district/create">
            ADD NEW
          </Button>
          <Divider>
            <Chip />

...
import { getService } from "../../../../../store/masterData/services/actions";
...
export default function ListServiceComponent(props: ServiceProps) {
  const { services } = props;
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const rows = services.sort((a, b) => (a.name < b.name ? -1 : 1));
  var number = 0;
  ...
  const handleViewById = (id: any) => {
    dispatch(getService(id,history));
  };
    ...
    <Grid item xs={12}>
      <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
        ...
        <Root sx={{ width: 1200, maxWidth: "100%" }}>
          <Button variant="outlined" size="small" href="/admin/service/create">
            ADD NEW
          </Button>
          <Divider>
            <Chip />
